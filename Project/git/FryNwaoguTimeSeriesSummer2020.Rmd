...
We model daily positive percent with ana aruma model. The parzen window shows a frequency at about 0.14 so this suggegsts a period of about 7 so we differenced the data and resulting residual is consistent with white noise. We forecast a 7 day and 90 days to use for our multivariate model.
```{r}
plotts.sample.wge(dataGrouped$positive_perc)
...
```
###### Daily Total Test
We model daily total test using signal plus noise because as you can se that the realization has an upward trend. We fit a trend as the independent variable with the daily total test as the dependent variable. We forecast a 7 day and 90 days to use for our multivariate model.
```{r}
...
```
###### Daily Negative Test
We model negative test using arima model because there is a trend in the data with some frequency. After differencing the trend, the residual seem to be consistent with white noise. We forecast a 7 day and 90 days to use for our multivariate model.
```{r}
...
hist(ASEHolder)
WindowedASE_MLP = mean(ASEHolder)
...
#### VAR Model (Percent change can be model but can not generate forecasts when the entire dataset is used)
```{r}
...
for( i in 1:(len-(trainingSize + horizon) + 1))
{
CoroVar = VAR(cbind(dataGrouped$positiveIncrease[i:(i+(trainingSize-1))],dataGrouped$negativeIncrease[i:(i+(trainingSize-1))],dataGrouped$totalTestResults[i:(i+(trainingSize-1))],dataGrouped$hospitalizedCurrently[i:(i+(trainingSize-1))]), type = "both", lag.max = 10) 
...
CoroVar = VAR(cbind(smallData$positiveIncrease,smallData$negativeIncrease,smallData$totalTestResults,smallData$hospitalizedCurrently), type = "both", lag.max = 10)
...
preds = predict(Corofit, newxreg = DataForeDF)
...
plot(seq(1,len,1), dataGrouped$positiveIncrease, type = "l",xlim = c(0,len+7), ylim=c(0,100000),  ylab = "New Daily Cases", main = paste(as.character(7),"Days Forecast"))
...
plot(seq(1,len,1), dataGrouped$positiveIncrease, type = "l",xlim = c(0,len+90), ylim=c(0,350000),  ylab = "New Daily Cases", main = paste(as.character(90),"Days Forecast"))
lines(seq(len,(len+89),1), predsFinal, type = "l", col = "red")
```
##### Ensemble model
```{r}
ASE = (MLP_DF$ASE + VAR_DF$ASE + MLR_DF$ASE)/3

WindowedASE = (MLP_DF$WindowedASE + VAR_DF$WindowedASE + MLR_DF$WindowedASE)/3
