...
fore.mlp = forecast(fit.mlp, h = 10, xreg = CMDF)
plot(fore.mlp)
...
# My data set
aenergy = read.csv("~/Documents/datascience/DS6373/Dataset/Appliance_Energy/energydata_complete.csv", header = TRUE)

length(aenergy$date)
small = aenergy$Appliances[19000:19735]
plotts.sample.wge(small)
factor.wge(small)

?mlp
fit.mlp = mlp(ts(small),reps = 20,comb = "mean",hd.auto.type = 'cv',difforder = 52)
fit.mlp
plot(fit.mlp)
fore.mlp = forecast(fit.mlp, h = 20)
plot(fore.mlp)

ASE = mean((small[716:735] - fore.mlp$mean)^2)
ASE

#ARMA
x.mle = aic.wge(small)
mlefc = fore.arma.wge(small, phi = x.mle$phi, lastn = T, n.ahead = 20)
ASE = mean((mlefc$f - small[716:735])^2)
ASE